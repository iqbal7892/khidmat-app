...
class PairItems extends Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    this.state = {
      dataProvider: this.dataProvider.cloneWithRows(this._generateArray()),
      refreshing:false
    };
  ...
  _generateArray = () => {
    ...
    if (this.props.PairDataType === 'Favorites') {
      return this.props.data.filter((item: Pair) => this.props.favPairs.includes(item.id) && item.type == 0 && (this.props.isMarginPairs ? item.isMarginEnabled : true))
    } else if (this.props.PairDataType === 'Leverage') {
      return this.props.data.filter((item: any) => item.leveragePair == true && item.type == 0 && (this.props.isMarginPairs ? item.isMarginEnabled : true))
    } else if (this.props.PairDataType === 'Usdc') {
      return this.props.data.filter((item: any) => item.marketCurrency.symbol == 'USDC' && item.type == 0 && (this.props.isMarginPairs ? item.isMarginEnabled : true))
    }
    else if (this.props.PairDataType === 'Btc') {
      return this.props.data.filter((item:any) => item.marketCurrency.symbol == 'BTC' && item.type == 0 && (this.props.isMarginPairs ? item.isMarginEnabled : true))
    }
    else if (this.props.PairDataType === 'Usdt') {
      return this.props.data.filter((item:any) => item.marketCurrency.symbol == 'USDT' && item.type == 0 && (this.props.isMarginPairs ? item.isMarginEnabled : true))
    }
    else {
...
const mapStateToProps = (state: any) => {
  return {
    ...
    data: getFilterPairs(Object.values(state.pairReducer.pairs).filter((obj: any) => obj.Status === 1), state.pairReducer.inputSearch),
