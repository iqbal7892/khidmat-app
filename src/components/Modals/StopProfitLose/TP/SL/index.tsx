...
import OrderPercentage from "../../../../Trade/BuySell/OrderPercentage";
...
class TpSl extends Component<Props, State> {
                    ...
                    <View style={{ flex: 1 }}>
                        ...
                        <View style={[styles.flex, { paddingVertical: HeightToDp(7) }]}>
                            <View style={{ flex: 1, marginTop: HeightToDp(2), maxWidth: WidthToDp(70, true), marginRight: WidthToDp(5) }}>
                                <OrderTypeModal formGroupStyle={{ height: HeightToDp(42) }} orderType='Trigger Price' setOrderType={this.props?.setOrderType} />
                            </View>
                            <View style={{ flex: 1, marginTop: HeightToDp(2), maxWidth: WidthToDp(30, true) }}>
                                <OrderTypeModal formGroupStyle={{ height: HeightToDp(42) }} orderType='Trigger Type' setOrderType={this.props?.setOrderType} />
                            </View>
                        </View>
                        <View style={[styles.flex, { paddingVertical: HeightToDp(7) }]}>
                            <View style={{ flex: 1, marginTop: HeightToDp(2), maxWidth: WidthToDp(70, true), marginRight: WidthToDp(5) }}>
                                <OrderTypeModal formGroupStyle={{ height: HeightToDp(42) }} orderType='Market Price' setOrderType={this.props?.setOrderType} />
                            </View>
                            <View style={{ flex: 1, marginTop: HeightToDp(2), maxWidth: WidthToDp(30, true) }}>
                                <OrderTypeModal formGroupStyle={{ height: HeightToDp(42) }} orderType='Market Type' setOrderType={this.props?.setOrderType} />
                            </View>
                        </View>
                        <View style={styles.inputRangeSet}>
                            <Input
                                returnKeyType="next"
                                placeholder="Amount"
                                keyboardType="numeric"
                                placeholderTextColor={theme.colors._text.color1}
                                // value={this.state.model.Quantity}
                                // onBlur={this.updateModalQuantity}
                                iconRight={<Text style={styles.appendTxt}>BTC</Text>}
                                formGroupStyle={styles.formGroupStyles}
                                formControlWrapStyle={styles.formControlWrap}
                                style={[styles.formControl]}
                            />
                            <OrderPercentage setOrderPercentage={25} />
                        </View>
                        <View>
                            <View style={[styles.itemContainer, { paddingTop: HeightToDp(7) }]}>
                                <Text style={[styles.dLabel, { maxWidth: WidthToDp(90, true) }]} numberOfLines={1}>{__('position_amount')}</Text>
                                <Text style={[styles.dValue, { textAlign: 'right' }]}>0.001 BTC</Text>
                            </View>
                            <View style={[styles.itemContainer, { paddingTop: HeightToDp(7) }]}>
                                <Text style={[styles.dLabel, { maxWidth: WidthToDp(90, true) }]} numberOfLines={1}>{__('estimated_pnl')}</Text>
                                <Text style={[styles.dValue, { textAlign: 'right' }]}>--USDT</Text>
                            </View>
                        </View>
                    </View>
                    <View>
