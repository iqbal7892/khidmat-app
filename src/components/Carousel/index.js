...
export default class Carousel extends Component {
  static propTypes = {
    ...
    pageInfoBottomContainerStyle: PropTypes.any,
  ...
  constructor(props) {
    ...
    if (props.customWidth)
      this.customWidth = props.customWidth;
    if (props.customItemStyles)
      this.customItemStyles = props.customItemStyles;
    if (props.customWidthOffset)
      this.customWidthOffset = props.customWidthOffset;
  ...
  _setUpPages() {
    ...
    return pages.map((page, i) => (
      <Pressable style={[{ ...size }, pageStyle, this.customItemStyles]} key={`page${i}`}>
        {page}
      </Pressable>
    ));
  }
  ...
  _onLayout = (event) => {
    ...
    if (this.customWidth != null && this.customWidth)
  ...
  _calculateCurrentPage = (offset) => {
    ...
    const page = Math.round(offset / (width - this.customWidthOffset));
    return this._normalizePageNumber(page);
  }

  _calculateNextPage = (direction) => {
    const { width } = this.state.size;
    const ratio = this.offset / (width - this.customWidthOffset);
